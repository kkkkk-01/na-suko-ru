...
- ✅ 職員Web画面での応答・通話・履歴確認
- ✅ ブラウザ間WebRTC双方向音声通話（実機未登録時のフォールバック経路）
- ✅ **実機通話（Asterisk SIP）**: M5StickS3等のESP32実機と職員ブラウザの双方向音声通話（下記「実機通話アーキテクチャ」参照）
- ✅ FCMプッシュ通知（スマホへの着信通知・本番鍵設定済み）
...
### 未実装（次のステップ）
- ⬜ ESP32実機ファームウェア（仕様書は `docs/ESP32_FIRMWARE_SPEC.md` に完備。サーバー側の受け口は全て稼働済み）
- ⬜ 外部4G/5G回線の職員スマホ音声（TURNサーバー導入後）
...
## 実機通話アーキテクチャ（M5StickS3 ⇄ 職員）

施設PC 1台（docker compose）で完結する、施設運用に最も近い構成。音声呼は常に「職員→実機」方向で、実機は自動応答する。

```
① M5StickS3 ボタン ── REST POST /calls/request ──▶ API
② API ── Socket.IO + FCM ──▶ 職員（着信payloadに sip_extension 同梱）
③ 職員「応答」 ── JsSIP(WebRTC) ──▶ /sip-ws ──▶ Asterisk ── SIP/UDP ──▶ 実機内線1001
④ 実機は自動応答 → G.711 ulaw で双方向通話
```

- **実機側**: 素のSIP端末（UDP/5060、G.711 ulaw固定、WebRTC不使用）— ESP32に最適な業界標準構成
- **職員側**: JsSIP over WebSocket（WebRTC/DTLS-SRTP）。APIサーバー内の `/sip-ws` プロキシ経由でAsteriskに接続（1ポート集約でトンネル/nginx対応）
- **Asterisk 20**: 両側をG.711 ulawに統一しトランスコード不要（低CPU・音質劣化なし）
- **SIP内線**: 実機 1001【1003 / 職員 2001【2005（migration 003でDBとpjsip.confを同期）
- **構築手順**: `docs/VOICE_CALL_SETUP.md`（施設PC構築ガイド）/ 実機ファームウェア仕様: `docs/ESP32_FIRMWARE_SPEC.md` §4
- **テスト用内線**: `9999`（エコーテスト）/ `8888`（トーン再生）
